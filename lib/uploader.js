...
const ui = require('./ui');
const { getEditorConfigDir, detectEditorPaths, getInstalledExtensions } = require('./installer');
...
async function _uploadToSource(options = {}, localData) {
  ...
  if (mode === "merge" && fs.existsSync(remoteSettingsPath)) {
    try {
      const remoteObj = parseJsonSafe(
        fs.readFileSync(remoteSettingsPath, "utf8"),
      );
