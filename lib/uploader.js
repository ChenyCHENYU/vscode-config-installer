...
// deepMerge 和 parseJsonc 从 installer.js 导入，避免重复实现
...
async function _readLocalConfig(editorKey = 'vscode') {
  ...
  let extList = [];
  if (editorPaths) {
    const sp = ora({ text: `读取 ${editorLabel} 已安装扩展...`, indent: 4 }).start();
    extList = getInstalledExtensions(editorPaths);
    sp.succeed(`${editorLabel} 已安装 ${extList.length} 个扩展`);
  } else {
    console.log(
      chalk.yellow(`    ${ui.icons.warning} ${editorLabel} 未检测到，跳过扩展列表`),
    );
  }
