...
const REPO_URLS = {
  github: 'https://github.com/ChenyCHENYU/vscode-config.git',
  gitee: 'https://gitee.com/ycyplus163/vscode-config.git',
};
...
async function uploadConfig(options = {}) {
  ...
  if (!repoUrl) throw new Error(`不支持的源: ${sourceName}。可用: ${Object.keys(REPO_URLS).join(', ')}`);
  ...
  if (paths) {
    ...
    extList = getInstalledExtensions(paths);
