...
const REPO_URLS = {
  github:
    process.env.VSCODE_CONFIG_REPO_GITHUB ||
    'git@github.com:ChenyCHENYU/vscode-config.git',
  gitee:
    process.env.VSCODE_CONFIG_REPO_GITEE ||
    'git@gitee.com:ycyplus163/vscode-config.git',
};
...
async function uploadConfig(options = {}) {
  ...
  if (sourceName === 'all') {
    // 先读取一次本地配置，避免重复读
    const localData = await _readLocalConfig(options.editor);
    ...
    for (let i = 0; i < sources.length; i++) {
      const src = sources[i];
      ui.section(
        `${ui.icons.antenna} [${i + 1}/${sources.length}]`,
        `推送到 ${src.toUpperCase()}`
      );
      try {
        ...
        ui.warnBox(`${src.toUpperCase()} 推送失败`, [
          chalk.red(e.message),
          chalk.gray('继续推送其余源...'),
        ]);
      }
    ...
    const summaryLines = results.map(r =>
      r.ok
        ? `${ui.symbols.success} ${r.src.toUpperCase()}`
        : `${ui.symbols.error} ${r.src.toUpperCase()}  ${chalk.gray(r.msg.split('\n')[0])}`
    );
    summaryLines.push(
      '',
 ...
 * 读取本地编辑器配置（抽出复用）
 */
async function _readLocalConfig(editorKey = 'vscode') {
  const reg = EDITOR_REGISTRY[editorKey];
  const editorLabel = reg ? reg.label : 'VS Code';
  ui.section(ui.icons.search, `读取本地 ${editorLabel} 配置`);
  ...
  if (!fs.existsSync(settingsPath))
    throw new Error(`本地 settings.json 不存在: ${settingsPath}`);

  const localSettings = fs.readFileSync(settingsPath, 'utf8');
  ui.kv(
    'settings.json',
    `${(Buffer.byteLength(localSettings) / 1024).toFixed(1)} KB`
  );
  ...
  if (fs.existsSync(keybindingsPath)) {
    ...
    ui.kv(
      'keybindings.json',
      `${(Buffer.byteLength(localKeybindings) / 1024).toFixed(1)} KB`
    );
  } else {
  ...
  if (editorPaths) {
    const sp = ora({
      text: `读取 ${editorLabel} 已安装扩展...`,
      indent: 4,
    }).start();
    extList = getInstalledExtensions(editorPaths);
    sp.succeed(`${editorLabel} 已安装 ${extList.length} 个扩展`);
  } else {
    console.log(
      chalk.yellow(
        `    ${ui.icons.warning} ${editorLabel} 未检测到，跳过扩展列表`
      )
    );
  }
...
async function _uploadToSource(options = {}, localData) {
  ...
  if (!repoUrl)
    throw new Error(
      `不支持的源: ${sourceName}。可用: ${Object.keys(REPO_URLS).join(', ')}, all`
    );

  const { localSettings, localKeybindings, extList } = localData;

  // Clone 配置仓库
  ui.section(ui.icons.package, `同步 ${sourceName.toUpperCase()} 仓库`);
  ...
  if (!repoDir) {
    ...
    if (!fs.existsSync(path.join(repoDir, '.git')))
      throw new Error(`指定路径不是 Git 仓库: ${repoDir}`);
    const pullSp = ora({ text: '拉取最新...', indent: 4 }).start();
    try {
      git('pull --rebase', repoDir);
      pullSp.succeed('已拉取最新');
    } catch (_) {
      pullSp.warn('pull 失败，继续使用当前版本');
    }
  }
  ...
  if (mode === 'merge' && fs.existsSync(remoteSettingsPath)) {
    try {
      const remoteObj = parseJsonc(fs.readFileSync(remoteSettingsPath, 'utf8'));
      const localObj = parseJsonc(localSettings);
      ...
      console.log(
        `    ${ui.symbols.success} ${chalk.green('settings.json 合并完成（远程已有 key 保留，本地新增已追加）')}`
      );
    } catch (e) {
      // JSON 解析失败时回退到覆盖
      fs.writeFileSync(remoteSettingsPath, localSettings, 'utf8');
      console.log(
        `    ${ui.symbols.warning} ${chalk.yellow('settings.json 解析失败，已替换为本地版本')}`
      );
    }
  } else {
    fs.writeFileSync(remoteSettingsPath, localSettings, 'utf8');
    console.log(
      `    ${ui.symbols.success} ${chalk.green('settings.json 已覆盖')}`
    );
  }
  ...
  if (localKeybindings) {
    fs.writeFileSync(
      path.join(repoDir, 'keybindings.json'),
      localKeybindings,
      'utf8'
    );
    console.log(
      `    ${ui.symbols.success} ${chalk.green('keybindings.json 已写入')}`
    );
  }
  ...
  if (extList.length > 0) {
    ...
    if (mode === 'merge' && fs.existsSync(remoteExtPath)) {
      const remoteExts = fs
        .readFileSync(remoteExtPath, 'utf8')
        .split('\n')
        .map(l => l.trim())
      ...
      console.log(
        `    ${ui.symbols.success} ${chalk.green(`extensions.list 合并完成 (远程 ${remoteExts.length} + 新增 ${newExts.length})`)}`
      );
    } else {
      // 覆盖模式：按类别生成新文件
      const header = `# Editor Extensions\n# 由 vscode-config upload 生成 (${new Date().toLocaleDateString('zh-CN')})\n# 共 ${extList.length} 个扩展\n\n`;
      const content =
        header +
        extList
      ...
      console.log(
        `    ${ui.symbols.success} ${chalk.green(`extensions.list 已覆盖 (${extList.length} 个扩展)`)}`
      );
    }
  }

  // Commit & Push
  ui.section(ui.icons.rocket, '提交并推送');
  ...
  try {
    ...
    if (!status) {
      console.log(
        `    ${ui.symbols.info} ${chalk.blue('无变更，远程已是最新')}`
      );
    } else {
      const msg =
        mode === 'override'
