...
/**
 * 批量安装扩展（单次 CLI 调用，避免多窗口弹出 + 修复 Windows .cmd 兼容性）
 */
async function installExtensions(extensions, options = {}) {
  ...
  console.log('');
  console.log(chalk.blue(`🔄 开始安装 ${newExtensions.length} 个新扩展 (批量模式)...`));
  ...
  // 构建单次批量安装命令: code --install-extension ext1 --install-extension ext2 ...
  const batchArgs = [];
  for (const ext of newExtensions) {
    batchArgs.push('--install-extension', ext);
  }
  
  const spinner = ora(`正在安装 ${newExtensions.length} 个扩展...`).start();
  
  // 单次 CLI 调用（shell: true 解决 Windows .cmd 文件兼容性）
  const totalTimeout = config.installTimeout * newExtensions.length;
  const batchResult = await new Promise((resolve) => {
    const child = spawn('code', batchArgs, {
      stdio: 'pipe',
      shell: true,
      windowsHide: true
    });
    ...
    const timer = setTimeout(() => {
      if (!settled) {
        settled = true;
        child.kill();
        resolve({ success: false, output, error: '安装超时' });
      }
    }, totalTimeout);
    ...
    child.stdout.on('data', (data) => {
      ...
      output += text;
      // 从输出中提取安装进度
      const installed = text.match(/was successfully installed/gi);
      if (installed) {
        progressCount += installed.length;
        spinner.text = `正在安装扩展 (${progressCount}/${newExtensions.length})...`;
      }
    });
    ...
    child.on('close', (code) => {
      ...
      if (!settled) {
        settled = true;
        resolve({ success: code === 0, output, error: errorOutput, exitCode: code });
      }
    });
  ...
  spinner.text = '验证扩展安装状态...';
  const afterInstalled = getInstalledExtensionsList();
  const afterInstalledLower = afterInstalled.map(e => e.toLowerCase());
  
  const successfullyInstalled = newExtensions.filter(ext => afterInstalledLower.includes(ext.toLowerCase()));
  const failedToInstall = newExtensions.filter(ext => !afterInstalledLower.includes(ext.toLowerCase()));
  
  const installed = successfullyInstalled.length;
  const failed = failedToInstall.length;
  
  if (failed === 0) {
    spinner.succeed(`${installed} 个扩展全部安装成功 ✓`);
  } else if (installed > 0) {
    spinner.warn(`${installed} 个安装成功，${failed} 个安装失败`);
  } else {
    spinner.fail('扩展安装失败');
  }
  
  // 构建失败扩展详情
  const failedExtensions = failedToInstall.map(id => {
    const info = parseExtensionId(id);
    return {
      id,
      author: info.author,
      name: info.name,
