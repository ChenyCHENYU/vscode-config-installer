...
function _findVsix(dir, extId) {
  ...
  try {
    ...
    const prefixed = files.find(f => f.toLowerCase().startsWith(`${lower}-`) && f.endsWith('.vsix'));
...
async function installExtensions(vscodePaths, extensions, options = {}) {
  ...
  // 解析离线目录优先级: --extensions-dir > 环境变量 > null
  const extensionsDir =
    options.extensionsDir || process.env.VSCODE_CONFIG_EXTENSIONS_DIR || null;
  if (extensionsDir && fs.existsSync(extensionsDir)) {
    console.log(chalk.gray(`   离线目录: ${extensionsDir}`));
  }

  // 第一轮：正常安装（在线 or 有 extensionsDir 则用 .vsix）
  ...
  // 全部失败 + 非离线模式 → 自动降级：尝试离线兜底
  if (result.failedCount === valid.length && !extensionsDir) {
    const fallbackDir = await _resolveOfflineDir();
    if (fallbackDir) {
      console.log('');
      console.log(chalk.blue(`${ui.icons.gear} 切换到离线 .vsix 安装...`));
      const retryResult = _runExtInstall(vscodePaths, valid, {
        extensionsDir: fallbackDir,
        timeoutSec,
        verbose,
      });
...
/**
 * 解析离线 .vsix 目录（交互提示，仅在全部失败时触发）
 */
async function _resolveOfflineDir() {
  // 检查环境变量（可能运行时才设置）
  const envDir = process.env.VSCODE_CONFIG_EXTENSIONS_DIR;
  if (envDir && fs.existsSync(envDir)) return envDir;

  console.log('');
  console.log(
    chalk.yellow.bold(
      `${ui.icons.warning} 所有扩展安装失败（可能处于内网环境）`
    )
  );
  console.log(chalk.gray('   如有离线 .vsix 文件，可指定目录重试'));
  console.log('');
  ...
  const response = await prompts({
    type: 'text',
    name: 'dir',
    message: '.vsix 离线目录路径（留空跳过）',
  });

  if (!response.dir || !response.dir.trim()) return null;
  const dir = path.resolve(response.dir.trim());
