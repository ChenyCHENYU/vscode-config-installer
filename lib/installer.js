...
function httpGet(url, timeout = 15000, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(
      ...
      {
        timeout,
        headers: { "User-Agent": "vscode-config/3.5" },
      },
      (res) => {
        if (
          res.statusCode >= 300 &&
...
async function installSettings(configDir, mode, editorLabel = "VS Code") {
  ...
  if (mode === "merge" && fs.existsSync(fp)) {
    ...
    console.log(chalk.green(`✓ ${editorLabel} 设置合并完成（保留个人配置）`));
  } else {
    fs.writeFileSync(fp, content, "utf8");
    console.log(chalk.green(`✓ ${editorLabel} 设置安装完成`));
  }
}
...
async function installConfig(options = {}) {
  ...
  if (options.source) {
    ...
    const matched = CONFIG_SOURCES.filter(
