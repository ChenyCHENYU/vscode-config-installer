/**
 * @agile-team/vscode-config — 核心安装逻辑
 *
 * 绕过 code/code.cmd，直接调用 Electron + cli.js
 * ELECTRON_RUN_AS_NODE=1 → 零弹窗、stdout 可靠、跨平台
 * 支持 VS Code / Cursor / Windsurf / Kiro 多编辑器
 */
...
const { resolveSources } = require('./config');

// ─── 配置源 ────────────────────────────────────────────────────

let _activeSources = resolveSources();
...
async function fetchFile(filePath) {
  ...
  // 远程全部失败 → 尝试读取包内 defaults/ 兜底
  const localPath = path.join(__dirname, '..', 'defaults', filePath);
  if (fs.existsSync(localPath)) {
    if (spinner) spinner.warn(`${filePath} 远程不可用，使用本地离线配置`);
    return fs.readFileSync(localPath, 'utf8');
  }

  if (spinner) spinner.fail(`${filePath} 获取失败`);
  throw new Error(`所有配置源都不可用且无本地兜底: ${lastErr.message}`);
}
 ...
 * @returns {{ electron: string, cliScript: string, editor: string } | null}
 */
...
function detectEditorWindows(reg) {
  // 方法1: where <cmd>.cmd — 向上最多4层查找编辑器根目录
  // 部分编辑器（如 Cursor）的 cmd 脚本位于深层子目录（bin在resources/app/bin），
  // 需要逐级向上寻找含 exe 的根目录
  try {
    ...
    for (const line of lines) {
      let dir = path.dirname(line.trim());
...
/**
 * 以 ELECTRON_RUN_AS_NODE 模式执行编辑器 CLI
 * @param {{ electron: string, cliScript: string }} vscodePaths
...
/**
 * 获取编辑器用户配置目录路径
 * @param {string} editorKey - 编辑器标识
 * @returns {string} 配置目录绝对路径
 * @throws {Error} 未知编辑器时抛出
 */
...
/**
 * 解析 JSONC 文本（去除注释和尾逗号）
 * @param {string} text - JSONC 格式文本
...
/**
 * 深度合并两个对象（local 为基础，remote 覆盖同名 key）
 * @param {Record<string, any>} local
 * @param {Record<string, any>} remote
 * @returns {Record<string, any>} 合并后的新对象
 */
...
/**
 * 备份编辑器配置文件到 backup-{timestamp} 目录
 * @param {string} configDir - 编辑器用户配置目录
...
function backupConfig(configDir) {
  ...
  if (backed.length > 0) {
    ui.infoBox(
      '备份完成',
      [chalk.white(backed.join(', ')), chalk.gray(backupDir)],
      'blue'
    );
  }
  return backupDir;
...
/**
 * 校验扩展 ID 格式，防止命令注入
 * @param {string} id - 扩展 ID (如 esbenp.prettier-vscode)
 * @returns {boolean}
 */
function isValidExtId(id) {
  return /^[\w][\w.-]*$/.test(id);
}
...
/**
 * 自动检测伴侣包 @agile-team/vscode-config-extensions
 * 云桌面环境通过 npm install 获取 .vsix，主工具自动发现
 */
function _detectCompanionPkg() {
  try {
    const pkgJson =
      require.resolve('@agile-team/vscode-config-extensions/package.json');
    const dir = path.join(path.dirname(pkgJson), 'extensions');
...
/**
 * 在目录中查找与 ext-id 匹配的 .vsix 文件
 * 匹配规则: publisher.name.vsix 或 publisher.name-version.vsix
 * 找不到时返回原始 ext-id（回退到在线安装）
 */
function _findVsix(dir, extId) {
  if (!fs.existsSync(dir)) return extId;
  const lower = extId.toLowerCase();
  try {
    const files = fs.readdirSync(dir).filter(f => f.endsWith('.vsix'));
    // 精确匹配: publisher.name.vsix
    const exact = files.find(f => f.toLowerCase() === `${lower}.vsix`);
    if (exact) return path.join(dir, exact);
    // 前缀匹配: publisher.name-version.vsix
    const prefixed = files.find(
      f => f.toLowerCase().startsWith(`${lower}-`) && f.endsWith('.vsix')
    );
...
async function installExtensions(vscodePaths, extensions, options = {}) {
  ...
  // 解析离线目录优先级: --extensions-dir > 环境变量 > 伴侣包 > null (在线)
  const extensionsDir =
    options.extensionsDir ||
    process.env.VSCODE_CONFIG_EXTENSIONS_DIR ||
    _detectCompanionPkg() ||
    null;
  if (extensionsDir) {
    console.log(chalk.gray(`   离线目录: ${extensionsDir}`));
  }

  const result = _runExtInstall(vscodePaths, valid, {
    extensionsDir,
    timeoutSec,
    verbose,
  });

  return _verifyAndSummarize(
    vscodePaths,
    valid,
    result.failedList,
    alreadyCount,
 ...
 * 执行一轮扩展安装（在线或离线）
 */
function _runExtInstall(
  vscodePaths,
  valid,
  { extensionsDir, timeoutSec, verbose }
) {
  ...
  for (let i = 0; i < valid.length; i++) {
    ...
    const target = extensionsDir ? _findVsix(extensionsDir, ext) : ext;

    const sp = ora(`${tag} 安装 ${ext}...`).start();
    const result = runCli(
      vscodePaths,
      `--install-extension ${target} --force`,
      perExtTimeout
    );
    ...
    if (verbose) {
      ...
      console.log(
        chalk.gray(
          `   [verbose] ${ext}: ok=${result.ok}, output=${output.substring(0, 200)}`
        )
      );
      sp.start();
    }
    ...
    if (
      ...
      sp.fail(`${tag} ${ext}`);
      const errMsg =
        output.trim().substring(0, 150) || result.error || '未知错误';
      console.log(chalk.gray(`      原因: ${errMsg}`));
      failedList.push({ id: ext, error: errMsg });
    }
  }

  return { failedCount: failedList.length, failedList };
}

/**
 * 验证安装结果并输出摘要
 */
function _verifyAndSummarize(
  vscodePaths,
  valid,
  failedList,
  alreadyCount,
  ...
  for (const ext of valid) {
    if (afterSet.has(ext.toLowerCase())) {
      verified++;
    } else {
      verifyFailed++;
      const prev = failedList.find(f => f.id === ext);
      verifyFailedList.push({
        id: ext,
        error: prev ? prev.error : '验证未通过',
      });
    }
  }
  ...
  if (verifyFailed > 0) {
    ...
    if (verifyFailed === valid.length) {
      console.log('');
      ui.warnBox('所有扩展均安装失败（可能处于内网环境）', [
        chalk.white('请尝试以下方案:'),
        '',
        chalk.green('方案 1:') +
          chalk.white(' 在有网环境下执行一次 vscode-config install'),
        chalk.gray('        扩展安装后本地持久保留，之后无需网络'),
        '',
        chalk.green('方案 2:') + chalk.white(' 使用离线 .vsix 目录'),
        chalk.gray(
          '        管理员: vscode-config download-extensions --output <目录>'
        ),
        chalk.gray(
          '        用户:   vscode-config install --extensions-dir <目录>'
        ),
      ]);
    } else {
      console.log('');
      console.log(chalk.yellow(`${ui.icons.memo} 手动安装失败的扩展:`));
...
async function installConfig(options = {}) {
  // 重置配置源（防止多次调用时污染）
  _activeSources = resolveSources();

  // 配置源
  if (options.source) {
    const lower = options.source.toLowerCase();
    const allSources = resolveSources();
    const matched = allSources.filter(s => s.name.toLowerCase() === lower);
    if (matched.length > 0) {
      _activeSources = [
        ...matched,
        ...allSources.filter(s => !matched.includes(s)),
      ];
    } else {
      console.log(
        chalk.yellow(
  ...
  for (const editorKey of editorKeys) {
    ...
    if (options.dryRun) {
      ...
      console.log('');
      ui.warnBox('预览完成', [
        chalk.white('未做任何更改，加 --force 执行安装'),
      ]);
      continue;
    }
