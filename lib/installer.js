...
async function fetchFromSources(path, parser = (data) => data) {
  ...
  for (let i = 0; i < CONFIG_SOURCES.length; i++) {
    ...
    try {
      ...
      // 只有第一个源才显示 "获取配置"，后续源显示 "切换到备用源"
      if (isFirstSource) {
        currentSpinner = ora(`获取 ${path}...`).start();
      } else {
        if (currentSpinner) currentSpinner.text = `切换到 ${source.name}...`;
      }
      ...
      if (isLastSource) {
        ...
        // 还有备用源，继续尝试（不显示失败信息）
        if (currentSpinner) {
          currentSpinner.text = `${source.name} 不可用，切换备用源...`;
        }
        // 短暂延迟让用户看到切换信息
...
async function getExtensionList() {
  try {
    ...
    try {
      ...
      const spinner = ora('未找到 extensions.json，尝试 extensions.list...').start();
      ...
      try {
        ...
        spinner.succeed('extensions.list 获取成功 ✓');
        return extensionLines;
      } catch (listError) {
        spinner.fail('extensions.list 获取失败');
        throw listError;
      }
    }
  } catch (error) {
...
async function installExtensions(extensions, options = {}) {
  ...
  console.log(chalk.gray(`   配置文件包含: ${extensions.length} 个扩展`));
  
  // 延迟显示，让信息逐步出现
  await new Promise(resolve => setTimeout(resolve, 200));
  console.log(chalk.green(`   本地已有: ${alreadyInstalled.length} 个 (无需重复安装)`));
  
  await new Promise(resolve => setTimeout(resolve, 200));
  console.log(chalk.yellow(`   需要新装: ${newExtensions.length} 个`));
  console.log('');
  
  if (alreadyInstalled.length > 0) {
    await new Promise(resolve => setTimeout(resolve, 300));
    console.log(chalk.green('✅ 本地已有扩展 (跳过安装):'));
    
    // 逐个显示扩展，但速度较快
    const displayExtensions = alreadyInstalled.slice(0, 5);
    for (let i = 0; i < displayExtensions.length; i++) {
      console.log(chalk.gray(`   • ${displayExtensions[i]}`));
      if (i < displayExtensions.length - 1) {
  ...
  if (newExtensions.length === 0) {
    await new Promise(resolve => setTimeout(resolve, 300));
    console.log(chalk.green('🎉 所有配置扩展都已存在，无需额外安装！'));
  ...
  // 显示安装结果
  console.log('');
  console.log(chalk.green(`✅ 本次新装: ${installed} 个扩展`));
  console.log(chalk.blue(`📋 本地已有: ${alreadyInstalled.length} 个扩展`));
...
async function installConfig(options = {}) {
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 100));
    console.log(chalk.gray(`  • 配置包含扩展: ${extensionResults.total} 个`));
    
    await new Promise(resolve => setTimeout(resolve, 100));
    console.log(chalk.gray(`  • 本次新装扩展: ${extensionResults.installed} 个`));
    ...
    if (extensionResults.failed > 0) {
      console.log('');
      await new Promise(resolve => setTimeout(resolve, 200));
      console.log(chalk.yellow('💡 关于扩展处理:'));
      console.log(chalk.gray('  • 🛡️  您现有的扩展都会保留'));
      console.log(chalk.gray('  • 📦 只安装配置中新增的扩展'));
      console.log(chalk.gray('  • ⚙️  扩展个人设置可能需要重新配置'));
      console.log(chalk.gray('  • 💾 可从备份中恢复个人设置'));
    } else if (extensionResults.installed === 0 && extensionResults.skipped > 0) {
      console.log('');
      await new Promise(resolve => setTimeout(resolve, 200));
      console.log(chalk.green('💡 扩展处理完成:'));
      console.log(chalk.gray('  • 🛡️  您的所有现有扩展都已保留'));
      console.log(chalk.gray('  • 📦 配置中的扩展在本地都已存在'));
      console.log(chalk.gray('  • ✨ 无需安装任何新扩展'));
    } else {
      console.log('');
      await new Promise(resolve => setTimeout(resolve, 200));
      console.log(chalk.green('💡 扩展处理完成:'));
      console.log(chalk.gray('  • 🛡️  您的所有现有扩展都已保留'));
      console.log(chalk.gray('  • 📦 配置中的扩展已确保安装'));
    }
