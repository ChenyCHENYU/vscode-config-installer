...
const EXTENSION_INSTALL_CONFIG = {
  maxConcurrent: 1,           // 串行安装（避免多窗口和spinner冲突）
  maxRetries: 1,              // 最大重试次数
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const result = await new Promise((resolve) => {
      ...
      child.on('close', (code) => {
        ...
        if (!settled) {
          settled = true;
          resolve({ success: code === 0, output, error: errorOutput || output, attempt });
        }
      });
...
async function installExtensions(extensions, options = {}) {
  ...
  for (let i = 0; i < newExtensions.length; i++) {
    ...
    try {
      ...
      if (result.success) {
        ...
        spinner.succeed(`${progress} ${extensionInfo.displayName} ✓${retryText}`);
        installed++;
      } else {
        const retryText = result.attempt > 0 ? ` (重试 ${result.attempt + 1} 次后仍失败)` : '';
        spinner.fail(`${progress} ${extensionInfo.displayName} ✗${retryText}`);
        failed++;
  ...
  // 安装后批量验证（一次性检查，非逐个调用）
  if (installed > 0) {
    const verifySpinner = ora('验证扩展安装状态...').start();
    const currentInstalled = getInstalledExtensionsList();
    const currentInstalledLower = currentInstalled.map(e => e.toLowerCase());
    const notVerified = newExtensions.filter(ext =>
      !currentInstalledLower.includes(ext.toLowerCase()) &&
      !failedExtensions.some(f => f.id === ext)
    );
    
    if (notVerified.length > 0) {
      verifySpinner.warn(`${notVerified.length} 个扩展可能需要重启 VSCode 后生效`);
    } else {
      verifySpinner.succeed('所有扩展验证通过 ✓');
    }
  }
