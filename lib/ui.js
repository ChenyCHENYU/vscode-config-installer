...
function createTheme(colorizer) {
  return {
    multiline(text) {
      return text
        .split('\n')
...
const theme = createTheme(chalk.cyanBright);
const successTheme = createTheme(chalk.greenBright);
const warnTheme = createTheme(chalk.yellowBright);
...
function extSuccess(tag, name) {
  console.log(
    `    ${chalk.green('✔')} ${chalk.gray(tag)} ${chalk.white(name)}`
  );
}
