...
const EDITOR_REGISTRY = {
  ...
  cursor: {
    ...
    linuxDirs: [
      '/usr/share/cursor',
      '/opt/cursor',
      '/snap/cursor/current/usr/share/cursor',
    ],
    linuxExeName: 'cursor',
  ...
  qoder: {
    label: 'Qoder',
    winExeName: 'Qoder.exe',
    winCmdScript: 'qoder.cmd',
    winDirs: [
      () => path.join(process.env.LOCALAPPDATA || '', 'Programs', 'Qoder'),
    ],
    winPathPattern: /qoder/i,
    macAppNames: ['Qoder.app'],
    macExeName: 'Qoder',
    linuxDirs: ['/usr/share/qoder', '/opt/qoder'],
    linuxExeName: 'qoder',
    linuxWhichCmd: 'which qoder',
    configDirName: 'Qoder',
    cliName: 'qoder',
...
// 合并配置文件中的自定义编辑器
const { resolveCustomEditors } = require('./config');
const customEditors = resolveCustomEditors();
if (customEditors) {
  for (const [key, entry] of Object.entries(customEditors)) {
    if (!EDITOR_REGISTRY[key]) {
      EDITOR_REGISTRY[key] = entry;
    }
  }
}
 ...
 * @param {string} [editorOpt] - 编辑器名称 (vscode|cursor|windsurf|kiro|all)
 * @returns {string[]} 编辑器 key 列表
 */
